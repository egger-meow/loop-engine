...
的正典檔案、固定的格式與更新節奏。Agent 讀檔案而不是問你;要轉向一個正在跑的
迴圈,直接跟它說就好——迴圈會把你說的話折進那些正典檔案,而不是讓它蒸發在
scrollback 裡。
...
沒有另外的信箱檔——你直接跟迴圈說話來轉向它。隨時說一句修正、新需求、
「方向錯了」,agent 會:

1. 分類它(task 級 → 改佇列;方向級 → 彈回 phase loop;事實修正 → 改現況
   文件;問題 → 直接回答);
2. **在同一輪裡翻譯進正典檔案**——那個 commit 就是「你的話被理解成什麼」
   的回執。
...
Agent 自己迴圈,`ROADMAP.md` 是你花授權的地方,聊天是你轉向的地方,
commit diff 是你稽核的地方。
...
- **在授權內規劃,永不自我授權。** agent 的規劃權力被人類已寫進 `ROADMAP.md`
  的內容框住。
- **轉向就是人類已經說過的話。** 聊天指令適用跟其他事一樣的分類、翻譯紀律
  ——一被採取行動就折進正典檔案——所以之後可稽核的是 diff,不是 scrollback。
...
**怎麼在 agent 跑到一半時改方向?**
直接在聊天裡說。Task 級的意見會被折進佇列、不打斷節奏;方向級的意見會把
agent 彈回 phase loop 重新規劃。無論哪種,結果的 commit 裡就包含你那句話
