...
- 除非傾倒已經回答,否則永遠要包含兩題:**填好的文件要用什麼語言寫**,以及
  **這個系統絕對不能做什麼**——傾倒幾乎從不主動說出護欄,而護欄是整個 repo
  裡權威最高的內容。
- 如果傾倒用主觀或美感字眼(「現代」「簡潔」「親切」「直覺」)描述任何第一
  個 phase 會讓人看到的東西——介面、語氣、輸出格式——就問一個具體錨點:一
  個參考的 app/網站、一個範例,或「比較像 X 而不是 Y」的對比。沒有錨點,這
  種字眼就會變成 agent 自己挑,挑錯了要等人類看到跑起來的結果才會被抓到—
  —現在錨定,比第一個 phase 做完後重來便宜。這一題仍然算進約 10 題的上限,
  不會放寬上限,而且大多數傾倒根本不會觸發它。
