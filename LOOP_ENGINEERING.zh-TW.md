...
PHASE LOOP(大迴圈)
  ...
  4. 史上第一次出現非 N/A 的「Try It Yourself」? ── 是 → 等待人類(僅一次)
  5. 啟用下一個已授權的 phase → 拆解進 PRIORITIES.md
  6. ↓ 執行 task loop
  7. ↑ 回到步驟 1

TASK LOOP(小迴圈,在步驟 5 內部,佇列還有項目就持續重複)
  a. 把聊天輸入折進來 ──────────────── 方向級輸入 → 退出到 phase loop
...
2. **聊天裡有方向級的輸入**——目標改變、策略轉向、「這個方向不對」。不要試
...
聊天裡一則小小的人類備註(例如「順便修一下 pager 那個差一的錯誤」)**不算**
...
1. **先把聊天輸入折進來。** 方向級的項目會落在這裡——先把它們套用到
...
3. **收尾一個做完的 phase。** 如果啟用中的 phase 的退場條件看起來已經達成:
   ...
   phase 保持啟用中。
4. **第一眼檢查。** 完整規則見下方「人類驗收」——簡言之,如果剛寫好的這份
   audit,是這個專案第一份「Try It Yourself」段落是真的(不是「N/A」)的
   audit,就停在這裡等人類,先不要進到步驟 5。這件事每個專案最多只發生一
   次;一旦有任何一個 phase 通過這個檢查,之後就直接跳到步驟 5。
5. **啟用下一個已授權的 phase。** 取 `ROADMAP.md` 裡「Authorized Phases」下
   的第一個 phase,標記為啟用中,並把它拆解成具體的 `PRIORITIES.md` 項目
   ——每一項都要有清楚的「done means」,讓 task gate 能夠驗證。這個拆解是在
   人類已經授權的範圍內做機械式規劃,所以不需要逐項簽核。
6. **執行 task loop**,直到出現退場觸發條件。
7. **回到步驟 1。**

## 人類轉向

沒有信箱檔——人類轉向一個正在跑的迴圈,靠的是直接跟它說話,就在 agent 已經
在工作的同一個聊天裡。人類說的話會被套用跟一則書面備註一樣的分類方式,然後
在迴圈繼續前折進去:

- **task 級**(一個 bug、一個小調整、一個小新增)→ 翻譯成 `PRIORITIES.md`
  的修改或直接修正;留在 task loop。
- **方向級**(目標改變、策略轉向、「方向不對」)→ 退出到 phase loop;套用到
  `ROADMAP.md`、`docs/project-charter.md` 或 `docs/system-direction.md`,或
  者如果需要只有人類能做的決定,就停下來問。
- **事實修正** → 修 `docs/status.md` 或受影響的文件。
- **問題** → 直接回答;不用改文件。

因為採取行動而產生的 commit 就是回執——跟這個 repo 其他地方一樣的「翻譯進
正典檔案」紀律,只是沒有實體信箱要清空。這是一個刻意的取捨:它假設聊天裡真
的有一個人在。一個無人監督運行的 agent——排程跑的、背景跑的、或是接續一段
沒人盯著的空檔——在這裡沒有聊天可讀,所以這套腳手架不提供那種情境的通道;
如果你需要,那是你專案自己要加的,不是這裡內建的。

## 框架回饋:飛行記錄器

[`FRAMEWORK_FEEDBACK.md`](FRAMEWORK_FEEDBACK.md) 是人類轉向的鏡像:聊天把
人類的輸入帶*進*專案;這個檔案把缺陷回報帶*出*專案,送回這個專案當初複製的
那套腳手架。當框架本身在迴圈中辜負了你——照著文件走還是迷了路、某條規則造成明
...
Phase gate 證明的是 agent 沒有謊報自己做了什麼——不證明人類真的喜歡這個
結果。這是兩個不同的問題。放著不管的話,一個 phase 可能光靠 agent 自己跑
的測試和 agent 自己主導的走查就到達「Complete」,而從頭到尾沒有一個人類
...
會停下來等的關卡;如果讓它變成阻擋性的,就會重新製造出這個 repo 本來要避
免的「先等批准才能繼續」瓶頸。

**唯一的例外:第一則真正的邀請要擋一次。** 一個 phase 收尾之前的一切,都
是紙上授權——charter、domain model、這個 phase 自己的退場條件,全部都是在
還沒有任何東西可以看的時候,以*描述*的形式被核准的。第一個「Try It
Yourself」不是「N/A」的 phase,是「書面核准」跟「真正想要的東西」第一次能
拿一個真實產物來對照,而不是拿對那個產物的描述來對照的時刻,也是抓出落差
成本最低的時刻——要在更多 phase 疊在同一個地基上*之前*抓,不是之後。所
以:在啟用下一個 phase 之前,先檢查 `docs/audits/` 裡有沒有更早的 audit 已
經有非「N/A」的「Try It Yourself」段落。如果沒有,而剛收尾的這個 phase
有,就停下來——先不要啟用下一個 phase,就算 `ROADMAP.md` 裡已經有更多已
授權的 phase 在排隊——把那個段落原原本本地呈現給人類,問一個直接的問題:
這個東西真的符合原本想要的樣子嗎?等到回覆才繼續。(如果大多數 phase 目前
都還沒有可觀察的表面,這個檢查就單純延續到下一個 phase 收尾時再做,不會現
在就觸發——這跟「讀取紀律」原本就允許的例外一樣:為了確認某個具體的過去主
張而開一份檔案,不是例行性地整份讀完 `docs/audits/`。)

這是一次性的成本,不是新增的逐 phase 關卡:一旦有任何一個 phase 通過這個檢
查,之後每個 phase 都回到上面的非阻擋式預設——方向是否對了已經被實際驗證
過一次,如果每個 phase 都要重新驗證一次,就會重新製造出這個 repo 本來要避
免的逐步批准瓶頸。(如果這時候 roadmap 也剛好用完了,就照下面 roadmap 用完
的做法處理就好——那個做法本來就涵蓋了這個情況。)

一則沒人看到的邀請等於沒發出去。它對第一個 phase 之後的每個 phase 而言,
真正需要被看見的那一刻,就是下面的 roadmap 用完。
...
- **Roadmap 用完了。** 沒有啟用中的 phase,`ROADMAP.md` 的「Authorized
  ...
  phase loop。)不要只回報佇列空了:把自從上次 roadmap 用完以來寫的那些
  audit 裡的「Try It Yourself」段落收集起來,原原本本地呈現出來——這是
  一則邀請,邀請人類確認做出來的東西真的符合想要的樣子,而不只是能動的證
  明——然後請人類要嘛確認,要嘛寫下一個 phase 的目標與退場條件。
- **這是這個專案第一次做出人類能實際看到的東西。** 見上方「人類驗收」——這
  個檢查每個專案只跑一次,不是每個 phase 都跑,而且只在「Try It Yourself」
  段落真的是實質內容時才算數。
- **聊天裡的指令需要人類決定**——它提議了一個新 phase、一次基於危險程度的重
  排,或一個書面文件解決不了的方向改變。
...
要讀完整個專案的歷史,也不需要重讀舊的聊天 scrollback,因為每一則重要的指
令,在被採取行動的當下就已經翻譯進那些檔案裡了。這就是這個 repo 真正的重點:
