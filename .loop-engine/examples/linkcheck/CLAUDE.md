...
There's no separate mailbox file for human input — instructions arrive in
this conversation. Before taking new work, fold in whatever the human said
most recently: task-level → a `PRIORITIES.md` edit or a direct fix;
direction-level → re-plan at the phase level; a factual correction → fix
the affected status doc; a question → just answer.

Read, in this order, if you haven't already this session:

1. [`PRIORITIES.md`](PRIORITIES.md) — the task you're authorized to work on
   next.
2. [`ROADMAP.md`](ROADMAP.md) — which phase that task belongs to, and what
   phase comes next when the queue drains.
3. [`docs/project-charter.md`](docs/project-charter.md) — mission, safety
   principles, and the documentation contract.
4. [`docs/domain-model.md`](docs/domain-model.md) — shared names; don't
...
verification gates): `../../LOOP_ENGINEERING.md`.
